...
public class ThemeFetcher {
    ...
    public Optional<String> getActiveThemeName() {
        final Map<String, String> configMap = this.extensionClient.fetch(
            ConfigMap.class, SystemSetting.SYSTEM_CONFIG).map(
            ConfigMap::getData).orElse(new HashMap<>());
        try {
            Map<String, Object> activeJson = JSONObjectUtils.parse(
                configMap.get("theme"));
